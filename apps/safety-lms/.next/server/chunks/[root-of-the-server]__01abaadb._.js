module.exports = [
"[project]/apps/safety-lms/.next-internal/server/app/api/stats/dashboard/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/apps/safety-lms/src/app/api/stats/dashboard/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { count: enrollmentsCount, error: enrollmentsError } = await supabase.from("enrollments").select("*", {
            count: "exact",
            head: true
        ...
        const { count: completedCount, error: completedError } = await supabase.from("enrollments").select("*", {
            count: "exact",
            head: true
        ...
        const { count: inProgressCount, error: inProgressError } = await supabase.from("enrollments").select("*", {
            count: "exact",
            head: true
        }).eq("user_id", profile.id).eq("status", "in_progress");
        if (inProgressError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__01abaadb._.js.map